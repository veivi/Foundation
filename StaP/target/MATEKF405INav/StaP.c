...
void inavStaP_InertialAcquire(struct IMUState *state)
{
  ...
  state->version = IMU_DATA_VERSION;
  state->flags = 0;
  ...
  state->rate = FLOAT_VECTOR(value.v[X], -value.v[Y], -value.v[Z]);
  state->flags |= IMU_RATE_VALID;
  ...
  state->acceleration = FLOAT_VECTOR(value.v[X]*cm, -value.v[Y]*cm, -value.v[Z]*cm);
  state->flags |= IMU_ACCEL_VALID;
}
